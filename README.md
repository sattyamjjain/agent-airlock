...
**Test suite:** 4,004 tests · **Coverage:** 87.05% · **v0.8.77**<br>
<sub>Coverage runs 3,992 of 4,004; the 12 excluded are 8 benchmark tests (not correctness tests) and 4 docker-marked (need a daemon).</sub>
<!-- TEST-BADGE-END -->
...
| **Matched-pair multi-harness prompt injection** · one planted README line, benign twin of identical shape · _last verified 2026-08-15_ · `python -m benchmarks.harness_injection` | **Null result: the injection did not work.** `claude-code` 2.1.233 and `codex` 0.147.0 each acted **0/6 injected, 0/6 benign** while fixing the bug 12/12 and running the suite 12/12 | **Not injection resistance** — the benign twin was ignored identically, so this is indifference to the delivery channel, not detection. n=6/arm/harness, 95% Wilson [0.0%, 39.0%] — a 10% bound needs n=35 (`--trials 18`) · [method, power, and the two inconclusive runs](docs/benchmarks/injection-multi-harness.md) |
