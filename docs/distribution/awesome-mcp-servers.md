...
```markdown
- [agent-airlock](https://github.com/sattyamjjain/agent-airlock) — in-process least-privilege decorator for AI tool calls; deny-by-default, PII masking, per-CVE presets, 4,288 tests.
```
...
4,288 tests. Listed under Tools/Clients since it wraps servers rather than being
one.
