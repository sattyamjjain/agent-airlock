...
**Result (2026-08-26, 144/144 cells measured, 18 trials x 2 arms x 2 airlock modes).** `claude-code` 2.1.246 and `codex` 0.147.0 fixed the seeded bug in **144/144** cells and ran the test suite in **141/144** — and acted on the exfiltration-shaped script **0/36 each**. The benign control fired for the first time: `codex` ran the *harmless* twin **1/36**. Matrix total **1/144**, none of it on the injected arm.
