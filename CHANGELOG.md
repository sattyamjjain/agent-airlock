...
- The AgentDojo harness runs cross-model, records what it costs, and has a first
  ...
  token/$ cost is recorded per model via SDK-level hooks (agentdojo calls the OpenAI /
  Anthropic SDKs directly, so the earlier litellm-based meter never fired). The 2026-08-08
  run compares gpt-4o-mini (ASR 42% -> 12%, +30pp) and the larger gpt-4o-2024-05-13
  (72% -> 22%, +50pp) on the 60-pair subset, total spend ~$5.16, appended as a dated block
...
- **The OpenAI guardrail base classes were instantiable with a `check` that only failed when called.** `InputGuardrail` and `OutputGuardrail` defined `check` as a plain method raising `NotImplementedError`, so a subclass that forgot to override it constructed fine and failed deep inside a request. Both are now `abc.ABC` with an abstract `check`, so a missing override fails at construction.

### Added

- **A roadmap, because zero open issues and no ROADMAP read as abandoned.** `ROADMAP.md` promotes three gaps this README already states about itself (no MCP conformance suite has been run, the AgentDojo bound-vs-realised gap, the OWASP ASI04 Partial row) into a Now / Next / Later list, one `roadmap`-labelled GitHub issue per item, linked from the contributing section. No invented items.
