...
- **Refresh the test count** to the current README TEST-BADGE at submission time (v0.8.86 = 4,322).

## Entry (list format)

```markdown
- [agent-airlock](https://github.com/sattyamjjain/agent-airlock) - in-process least-privilege decorator for AI tool calls; deny-by-default, PII masking, per-CVE presets, 4,322 tests.
```
...
no model call. 4,322 tests, mypy-strict, CI-gated.
