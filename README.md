...
**Test suite:** 2,487 tests · **Coverage:** 83.11% · **v0.8.4**
<!-- TEST-BADGE-END -->
...
| **Corpus per-category coverage** (v0.8.3 extension to corpus-bench) | v0.8.3 — adopts the HarnessAudit-Bench (arXiv:2605.14271) two-category taxonomy (`resource_access`, `info_transfer`) as a corpus schema extension; `CorpusEntry.violation_category` field + `CategoryCount` decision field; `airlock corpus-bench` reports per-category coverage in text/json/md; **NOT a reproduction of HarnessAudit-Bench** (artifacts not yet public — taxonomy adopted as schema, scoring is not) |
