...
**Test suite:** 4,256 tests · **Coverage:** 87.34% · **v0.8.85**<br>
<sub>Coverage runs 4,244 of 4,256; the 12 excluded are 8 benchmark tests (not correctness tests) and 4 docker-marked (need a daemon).</sub>
<!-- TEST-BADGE-END -->
...
| [**`policy_bundle.lock`**](docs/cli/policy-bundle-lock.md) | v0.6.0 — SHA-256 preset pinning with two-way drift detection; `airlock pack lock` + `airlock replay --bundle-lock`. **The Python API round-trips; the CLI does not** — `--verify` fails on all three shipped packs ([why](docs/cli/policy-bundle-lock.md#what-does-not-work)) |
...
| [**`airlock kill-switch`**](docs/cli/kill-switch.md) | v0.5.9 — HMAC-signed freeze broadcast with 2-of-3 quorum reset. **Primitives only**: no cross-process transport ships and the listener is not wired into the `@Airlock` call path, so freezing a fleet is integration you write ([scope](docs/cli/kill-switch.md#status-the-primitives-are-wired-the-delivery-is-not)) |
