...
- **Refresh the test count** to the current README TEST-BADGE at submission time (v0.8.87 = 4,353).

## Entry (list format)

```markdown
- [agent-airlock](https://github.com/sattyamjjain/agent-airlock) — in-process least-privilege decorator for AI tool calls; deny-by-default, PII masking, per-CVE presets, 4,353 tests.
```
...
4,353 tests. Listed under Tools/Clients since it wraps servers rather than being
one.
